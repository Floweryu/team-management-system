  ...
  <el-dialog :title="isEditButton ? '编辑' : '新增'" :visible="dialogVisible" :before-close="cancel">
    <el-card shadow="hover">
      <el-form :model="formData" ref="formData" :rules="rules" label-width="80px" label-position="right">
        ...
        <el-row type="flex" align="middle" class="row-bg">
          <el-col>
            <el-form-item prop="content" label="会议内容" size="mini">
              <el-input
                type="textarea"
        ...
        <el-row type="flex" align="middle" class="row-bg">
          <el-col>
            <el-form-item prop="place" label="会议地点" size="mini">
        ...
        <el-row type="flex" align="middle" class="row-bg">
          <el-col>
            <el-form-item prop="state" label="会议状态" size="mini">
  ...
  methods: {
    ...
    submitForm(formName) {
      if (this.isEditButton) {
        // 如果是编辑页面
        this.updateMeeting(formName)
      } else {
        this.addMeeting(formName)
      }
    },
    // 重置表单
    resetForm(formName) {
    ...
    addMeeting(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.formData.organizerId = localStorage.getItem('userId')
          this.$http.meeting
            .addMeeting(JSON.stringify(this.formData))
            .then(res => {
              if (res.code === 0) {
                this.$notify({
                  message: '添加成功',
    ...
    // 更新信息
    updateMeeting(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.formData.organizerId = localStorage.getItem('userId')
          this.$http.meeting
            .updateMeeting(JSON.stringify(this.formData))
            .then(res => {
              if (res.code === 0) {
                this.$notify({
                  message: '更新成功',
