    ...
    <el-card shadow="hover">
      <el-form :model="formData" ref="formData" :rules="rules" label-width="80px" label-position="right">
        <el-row type="flex" align="middle" class="row-bg">
          <el-col>
            <el-form-item prop="name" label="类别名称" size="mini">
        ...
        <el-row type="flex" align="middle" class="row-bg">
          <el-col>
            <el-form-item prop="description" label="类别描述" size="mini">
              <el-input
                type="textarea"
  ...
  methods: {
    ...
    submitForm(formName) {
      if (this.isEditButton) {
        // 如果是编辑页面
        this.updateClassify(formName)
      } else {
        this.addClassify(formName)
      }
    },
    // 重置表单
    resetForm(formName) {
    ...
    addClassify(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$http.classify
            .addClassify(JSON.stringify(this.formData))
            .then(res => {
              if (res.code === 0) {
                this.$notify({
                  message: '添加成功',
    ...
    // 更新信息
    updateClassify(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$http.classify
            .updateClassify(JSON.stringify(this.formData))
            .then(res => {
              if (res.code === 0) {
                this.$notify({
                  message: '更新成功',
