  ...
  <div class="header">
    <div class="left">
      <div class="select-classify">
        ...
        <el-select class="select" v-model="classifyValue" placeholder="请选择" size="mini" @change="selectChange">
          <el-option v-for="item in classifyList" :key="item.value" :label="item.label" :value="item.value"> </el-option>
        </el-select>
        <span class="tag">标签：</span>
        <el-select class="select" v-model="labelValue" placeholder="请选择" size="mini" @change="selectChange">
    ...
    <div class="right">
      <dt>排序：</dt>
      <dd :class="{ active: index === isSelect }" @click="changeColor(index)" v-for="(item, index) in sortedList" :key="index">
        {{ item.label }}
      </dd>
    </div>
  </div>
